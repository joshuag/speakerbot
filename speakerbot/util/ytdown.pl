...
my ($version) = ('$Revision: 1.448 $' =~ m/\s(\d[.\d]+)\s/s);
...
my %ciphers = (
  ...
  'en_US-vflIlILAX/html5player' => 'r w7 w19 w38 s3 w41 s1 r w1', # 04 Dec 2014
  'en_US-vflEegqdq/html5player' => 'r w7 w19 w38 s3 w41 s1 r w1', # 10 Dec 2014
  'en_US-vflkOb-do/html5player' => 'r w7 w19 w38 s3 w41 s1 r w1', # 11 Dec 2014
  'en_US-vfllt8pl6/html5player' => 'r w17 w33 w53',               # 16 Dec 2014
  'en_US-vflsXGZP2/html5player' => 's3 w38 s1 w16 r w20 w69 s2 w15',# 18 Dec 2014
  'en_US-vflw4H1P-/html5player' => 'w8 r s1',                     # 23 Dec 2014
);
...
sub guess_cipher(;$$) {
  ...
  if (! $cipher_id) {
    ...
    # Get random video -- pick one towards the end, because sometimes
    # the early ones are rental videos.
    ($id) = ($body =~ m@.*/watch\?v=([^\"\'/<>]+)@si);
    errorI ("no  videos found on home page $url") unless $id;
    $url .= "/watch\?v=$id";
...
sub scrape_youtube_url($$$$$$) {
  ...
  if ($verbose == 0 && $err && $err =~ m/livestream/sio) {
...
sub munge_title($) {
  ...
  $title =~ s/\b ( ( (in \s*)? 
                     (
                       HD | TV | HDTV | HQ | 720\s*p? | 1080\s*p? |
                       High [-\s]* Qual (ity)? 
                     ) |
                     FM('s)? |
